class Objectives {
  ...
  private drawText() {
    ...
    textSize(22);
    textWrap(WORD);
    text("The player with most keys when the time runs out wins!", width / 2 * 0.7, height / 2.4, 280)
    pop();
    ...
    textSize(22);
    textWrap(WORD);
    text("Watch out for monsters - they will freeze you!", width / 2 * 0.7, height / 1.9, 280)
    pop();
    ...
    textSize(22);
    textWrap(WORD);
    text("Pick up special items to get those keys quicker than your opponent!", width / 2 * 0.7, height / 1.56, 280)
    pop();
  ...
  private drawImages() {
    ...
    if (this.monsterdescriptions.width > 0 && this.monsterdescriptions.height > 0) {
        image(this.monsterdescriptions, width / 2 - -165, height / 2 - 15, 115, 105);
